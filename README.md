...
Scaniverseなどから書き出したGLBをブラウザ内で読み込み、机面と対象範囲を調整して体積を推定できます。GLBは端末外へ送信されません。

- [3Dモデルから体積を測る](https://satorumuro.github.io/hakarukun-web/volume/mesh/)
- 机面の自動推定と切断位置の調整
- 対象範囲の自動設定と手動調整
- 切断面を底面として閉じた体積（cm³ / mL）の推定
- 推定外形寸法と投影面積の表示

現在は、机などの平面上に置いた、上から見て重なりの少ない物体を対象にしています。深い凹み、大きなオーバーハング、スキャン欠損がある形状では誤差が増えます。
